defmodule BackendWeb.CoreComponents do
    ...
    >
      <div class={[
        ...
        <div class="flash-card__content">
          ...
          <p class="flash-card__body">{msg}</p>
        </div>
  ...
  def button(%{rest: rest} = assigns) do
    variants = %{
      ...
      nil => "md-button-secondary"
    }

    assigns =
      assign_new(assigns, :class, fn ->
        ["md-button", Map.fetch!(variants, assigns[:variant])]
      end)
      ...
      <label for={@id} class="checkbox-field">
        ...
        <span class="checkbox-field__label">
          <input
            ...
            value="true"
            checked={@checked}
            class={@class || "checkbox-field__input"}
            {@rest}
          />
          <span>{@label}</span>
        </span>
      ...
      <label for={@id} class="field-label">
        ...
        <select
          ...
          class={[
            ...
            @errors != [] && (@error_class || "field-input--error")
          ]}
          multiple={@multiple}
          {@rest}
        >
  ...
  def header(assigns) do
    ~H"""
    <header class={[@actions != [] && "section-heading", "section-heading section-heading--spaced"]}>
      <div class="section-heading__copy">
        <h1 class="section-title">
          {render_slot(@inner_block)}
        </h1>
        <p :if={@subtitle != []} class="section-subtitle">
          {render_slot(@subtitle)}
        </p>
      </div>
      <div class="section-heading__actions flex-none">{render_slot(@actions)}</div>
    </header>
    """
  end
        ...
        <tbody id={@id} phx-update={is_struct(@rows, Phoenix.LiveView.LiveStream) && "stream"}>
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)}>
            <td
              ...
              class={@row_click && "admin-table__cell--clickable"}
            >
              {render_slot(col, @row_item.(row))}
            </td>
            <td :if={@action != []} class="admin-table__actions">
  ...
  def list(assigns) do
    ...
    <ul class="detail-list">
      <li :for={item <- @item} class="detail-list__row">
